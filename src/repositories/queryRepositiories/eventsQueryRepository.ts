...
export const eventsQueryRepository = {
    filterEvents: async(): Promise<Array<EventType> | "Error"> => { //get events for event list
        try {
            const events = client.db("calendar").collection<EventType>("events").find({}).sort({}).toArray()
            return events
        } catch(err){
            return "Error"
        }
    },
    findEvents: async(year?: string, month?: string, day?: string): Promise<Array<EventType> | "Error"> => { //get events for month, year, and day views
        let queries = ''
        if(year && !month && !day){
            var query = {
                "start.year": {"$lte": year},
